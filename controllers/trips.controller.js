...
exports.getTripsData = async (req, res, next) => {
    try {
        const result = await mysql.execute("SELECT DATE_FORMAT(tripDate, '%Y%m%d') as date, SUM(tripKm) as tripKm, ROUND((SUM(tripTime)/60),2) as tripTime FROM trip_data GROUP BY date");
